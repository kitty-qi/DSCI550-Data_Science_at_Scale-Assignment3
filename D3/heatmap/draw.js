function drawMap(divclass, fpath) {
        ...
        d3.json(fpath, function(error, mockdata) {
        ...
        function processWorldD(world, data) {
                for(var idx=0; idx < data.aggregations.world_map.buckets.length; idx++) {
                    var cCode = data.aggregations.world_map.buckets[idx].country.toUpperCase();
                    var doc_count = data.aggregations.world_map.buckets[idx].count;
        ...
        document.getElementById('graph' + graphId).style.display = 'block';
      };*/
}
